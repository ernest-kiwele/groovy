...
class GinqAstWalker implements GinqVisitor<Object>, SyntaxErrorReportable {
    ...
    private static ConstructorCallExpression constructNamedRecordCtorCallExpression(List<Expression> expressionList) {
        ...
        ConstructorCallExpression namedListCtorCallExpression = ctorX(ClassHelper.make(NamedRecord.class), args(new ListExpression(elementExpressionList), new ListExpression(nameExpressionList)))
    ...
    private Expression correctVariablesOfGinqExpression(DataSourceExpression dataSourceExpression, Expression expr) {
        ...
        def correctVarsForJoin = { Expression expression ->
            ...
            /*
                     ...
                     */
            Expression prop = new VariableExpression(lambdaParamName)
            ...
            if (expression instanceof VariableExpression) {
                ...
                if (isGroup) { // groupby
                    if (expression.implicitThis) {
                        ...
                        if ('count' == methodName && ((TupleExpression) expression.arguments).getExpressions().isEmpty()) {
                        ...
                        } else if ('sum' == methodName && 1 == ((TupleExpression) expression.arguments).getExpressions().size()) {
                            visitingAggregateFunction = true
                            ...
                            transformedExpression =
                                    callXWithLambda(
                                            propX(new VariableExpression(lambdaParamName), 'v2'), 'sum',
    ...
    private Tuple2<String, Expression> correctVariablesOfLambdaExpression(DataSourceExpression dataSourceExpression, Expression lambdaCode) {
        ...
        if (dataSourceExpression instanceof JoinExpression || isGroup) {
            lambdaParamName = lambdaCode.getNodeMetaData(__LAMBDA_PARAM_NAME) ?: generateLambdaParamName()
