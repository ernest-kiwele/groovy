...
class QueryableCollectionTest {
    ...
    void testGroupBySelect4() {
        ...
        def result = from(persons).groupBy2(p -> p.gender)
    ...
    void testGroupBySelect5() {
        ...
        def result = from(persons).groupBy2(p -> new NamedTuple<>([p.gender, p.name], ['gender', 'name']))
                .select(e -> Tuple.tuple(e.v1.gender, e.v2.count())).toList()
